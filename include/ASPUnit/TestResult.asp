...
Class TestResult
	...
	Public Sub AssertEquals(vExpected, vActual)
		If vExpected <> vActual Then
			AddFailure NotEqualsMessage(vExpected, vActual)
		End	If
	End Sub

	' Build a message about a failed equality check
	Function NotEqualsMessage(vExpected, vActual)
		NotEqualsMessage = "預期結果為 " & CStr(vExpected) & " 但實際結果為  " & CStr(vActual) & " ，應該要相等。"
	End Function

	Public Sub AssertNotEquals(vExpected, vActual)
		If vExpected = vActual Then
			AddFailure "預期結果為 " & CStr(vExpected) & " 但實際結果為 " & CStr(vActual) & " ，應該要不相等。"
		End	If
	End Sub

	Public Sub AssertExists(vVariable)
		If IsObject(vVariable) Then
			If (vVariable Is Nothing) Then
				AddFailure sMessage &  "變數的型態為 " & TypeName(vVariable)
			End If
		ElseIf IsEmpty(vVariable) Then
			AddFailure sMessage & "變數的型態為 " & TypeName(vVariable) & " (未初始化)."
		End If
	End Sub
